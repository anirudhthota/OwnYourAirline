...
│   ├── scheduler.js        — Schedule CRUD, turnaround validation, banked departures, min aircraft calc, physical location projection (assumedStartLocation)
...
└── ui/                     — UI rendering (reads state, manipulates DOM)
    ...
    ├── views/              — Modular view panels (DashboardView, FleetView, RoutesView, NetworkView, AircraftDetailView, HubOperationsView, etc.)
